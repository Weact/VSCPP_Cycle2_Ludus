...
Vector3 Vector3::normaxVal(float flt_x, float flt_y, float flt_z) {
	...
	if (norme_v1 > norme_v2) {
		...
		if (norme_v1 == norme_v2) {
			...
			return Vector3(flt_x, flt_y, flt_z);
		}
	}
}

Vector3* Vector3::normaxAdr(Vector3* v) {
	float norme_v1 = sqrt(pow(this->mflt_X, 2) + pow(this->mflt_Y, 2) + pow(this->mflt_Z, 2));
...
Vector3& Vector3::normaxRef(Vector3& v)
{
	//sqrt(x² + y² + z²).
	float norme_v1 = sqrt(pow(this->mflt_X, 2) + pow(this->mflt_Y, 2) + pow(this->mflt_Z, 2));
	float norme_v2 = sqrt(pow(v.mflt_X, 2) + pow(v.mflt_Y, 2) + pow(v.mflt_Z, 2));
